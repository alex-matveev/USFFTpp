...
void plan<T, 2, FoldPolicy>::reorder_points() {
...
#pragma omp parallel for
    for (std::ptrdiff_t i = 0; i < static_cast<ptrdiff_t>(m_points.size()); i++) {
        std::size_t indx = static_cast<size_t>(
            (m_oversamplingFactor * m_N[1] * std::get<1>(m_points[i])) + m_oversamplingFactor * m_N[1] / 2
        );
        std::size_t indy = static_cast<size_t>(
            (m_oversamplingFactor * m_N[0] * std::get<0>(m_points[i])) + m_oversamplingFactor * m_N[0] / 2
        );
...
void plan<T, 2, FoldPolicy>::fill_first_occurrence_array() {
    ...
    m_firstOccurrenceArray[(m_oversamplingFactor * m_N[0]) * (m_oversamplingFactor * m_N[1])] = m_points.size();
    for (std::ptrdiff_t i = (m_oversamplingFactor * m_N[0]) * (m_oversamplingFactor * m_N[1]) - 1;
         i >= static_cast<ptrdiff_t>(m_di[0].first + 1);
         i--) {
...
void plan<T, 2, FoldPolicy>::undo_zero_padding(std::complex<T> *in, std::complex<T> *out) {
    ...
    for (std::ptrdiff_t i = 0; i < m_N[0]; i++) {
        for (std::ptrdiff_t j = 0; j < m_N[1]; j++) {
            out[i * m_N[1] + j] =
                in[m_strides[0] + (i + m_N[0] / 2 + m_N[0] % 2) * m_strides[1]
...
void plan<T, 2, FoldPolicy>::fftshift(std::complex<T> *data, std::array<std::size_t, 2> N) {
    ...
    for (int i = 0; i < N[0]; i++) {
        for (int j = 0; j < N[1]; j++) {
            ...
            temp_data[new_i * N[1] + new_j] = data[m_strides[0] + m_strides[1] * i + m_strides[2] * j];
        }
    }
...
#pragma omp parallel for
    for (std::ptrdiff_t i = 0; i < static_cast<ptrdiff_t>(m_oversamplingFactor * m_N[0]); i++) {
        for (std::ptrdiff_t j = -m_radius; j < 0; j++) {
            ...
            std::ptrdiff_t ind2 =
                m_strides[0] + m_strides[1] * ((i + m_oversamplingFactor * m_N[0]) % (m_oversamplingFactor * m_N[0]))
                + m_strides[2] * ((j + m_oversamplingFactor * m_N[1]) % (m_oversamplingFactor * m_N[1]));
            data[ind1] = data[ind2];
        }
        for (std::ptrdiff_t j = m_oversamplingFactor * m_N[1];
             j < static_cast<ptrdiff_t>(m_oversamplingFactor * m_N[1] + m_radius);
             j++) {
            std::ptrdiff_t ind1 = m_strides[0] + m_strides[1] * i + m_strides[2] * j;
            std::ptrdiff_t ind2 =
                m_strides[0] + m_strides[1] * ((i + m_oversamplingFactor * m_N[0]) % (m_oversamplingFactor * m_N[0]))
...
#pragma omp parallel for
    for (std::ptrdiff_t i = -m_radius; i < 0; i++) {
        for (std::ptrdiff_t j = -m_radius; j < (std::ptrdiff_t)(m_oversamplingFactor * m_N[1]) + m_radius; j++) {
            std::ptrdiff_t ind1 = m_strides[0] + m_strides[1] * i + m_strides[2] * j;
            std::ptrdiff_t ind2 =
                m_strides[0] + m_strides[1] * ((i + m_oversamplingFactor * m_N[0]) % (m_oversamplingFactor * m_N[0]))
                + m_strides[2] * ((j + m_oversamplingFactor * m_N[1]) % (m_oversamplingFactor * m_N[1]));
...
#pragma omp parallel for
    for (std::ptrdiff_t i = m_oversamplingFactor * m_N[0];
         i < static_cast<ptrdiff_t>(m_oversamplingFactor * m_N[0] + m_radius);
         i++) {
        for (std::ptrdiff_t j = -m_radius; j < (std::ptrdiff_t)(m_oversamplingFactor * m_N[1]) + m_radius; j++) {
            std::ptrdiff_t ind1 = m_strides[0] + m_strides[1] * i + m_strides[2] * j;
            std::ptrdiff_t ind2 =
...
template <typename T, typename FoldPolicy>
std::size_t plan<T, 2, FoldPolicy>::size_of_buffer() {
    return (m_oversamplingFactor * m_N[0] + 2 * m_radius) * (m_oversamplingFactor * m_N[1] + 2 * m_radius);
}

template <typename T, typename FoldPolicy>
int plan<T, 2, FoldPolicy>::nonunform_to_uniform_transform(
...
template <typename T, typename FoldPolicy>
int plan<T, 2, FoldPolicy>::uninform_to_nonuniform_transform(
    std::complex<T> *in, std::complex<T> *out, fourier_direction direction
) {
    size_t size = size_of_buffer();
    auto buffer = std::unique_ptr<std::complex<T>[]>(new std::complex<T>[size]());
...
template <typename T, typename FoldPolicy>
plan<T, 2, FoldPolicy>::plan(std::array<std::ptrdiff_t, 2> N, std::vector<std::tuple<T, T>> &points, T epsilon)
    : m_points(points),
      m_N(N),
      m_epsilon(epsilon),
      ...
      m_firstOccurrenceArray(
          std::make_unique<std::ptrdiff_t[]>((m_oversamplingFactor * m_N[0]) * (m_oversamplingFactor * m_N[1]) + 1)
      ),
    ...
    const std::array<int, 2> inembed          = { static_cast<int>(m_oversamplingFactor * (m_N[0] + m_radius)),
                                                  static_cast<int>(m_oversamplingFactor * (m_N[1] + m_radius)) };
    const std::array<int, 2> onembed          = { static_cast<int>(m_oversamplingFactor * (m_N[0] + m_radius)),
                                                  static_cast<int>(m_oversamplingFactor * (m_N[1] + m_radius)) };
